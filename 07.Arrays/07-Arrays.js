...
function dosEnUno(arr1, arr2){
    ...
    conjuntoArr=arr1.concat(arr2)
    nuevoAr=[];
    for (let i = 0; i < conjuntoArr.length; i++) {
        if (nuevoAr.includes(conjuntoArr[i])){
            continue;
        }else{
            nuevoAr.push(conjuntoArr[i])
        }
